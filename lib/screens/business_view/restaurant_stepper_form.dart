...
import 'package:smart_promotion_of_tourism/models/business_restaurant.dart';
import 'package:smart_promotion_of_tourism/services/request.dart';

class ResStepperForm extends StatefulWidget {
  @override
  _ResStepperFormState createState() => _ResStepperFormState();
}

class _ResStepperFormState extends State<ResStepperForm> {
  ...
  List<String> _restType=<String>[
    'Sit Down',
    'Cafe',
    'Fast Food',
  ];
  List<String> _cuisineTypes = <String>[
    'Café',//0
    'African',
    'American',
    'Asian',
  ...
  saveBusiness(context) async{
    ...
    BusinessRestaurant busRest = BusinessRestaurant(
        position: point.data,
        longitude: long,
        latitude: lat,
    ...
    try {
      await _business.doc().set(busRest.toMap());
        ...
        child: Column(
          children: [
            Padding(
              ...
              child: Text("Type: Food & Beverages"),
            ),
            Text("Name: "+_name),
            Padding(
              padding: const EdgeInsets.all(8.0),
            ...
            Text("Address: "+_address +", " +cityValue +", " +stateValue +", " +countryValue),
            Padding(
              padding: const EdgeInsets.all(8.0),
            ...
            Text("Category: " +_category.toString()),
            Text("Cuisines: " +_cuisine.toString()),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Food & Beverages Form'),
      ),
    ...
    return Column(
      children: <Widget>[
        ...
        Row(
          ...
          children: [
            DropdownButton(
              items: _restType.map((value) => DropdownMenuItem(
                child: Text(value),
                value: value,
              )).toList(),
              onChanged: (val){
    ...
    return Column(
      children: <Widget>[
        ...
        SearchableDropdown.multiple(
          items: _cuisineTypes.map((value) => DropdownMenuItem(
            child: Text(value),
            value: value,
          )).toList(),
          selectedItems: selectedTypes,
          hint: "Select up to 3 items",
          searchHint: "Select up to 3 items",
          validator: (selectedItemsForValidator) {
            if (selectedItemsForValidator.length > 3) {
              return ("Select up to 3 types");
            }
            return (null);
          },
          onChanged: (value) {
